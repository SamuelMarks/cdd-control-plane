...
fn hash_password(password: &str) -> Result<String, crate::error::Error> {
    ...
    let hash = argon2.hash_password(password.as_bytes(), &salt).map_err(|_| crate::error::Error::InternalError)?;
    Ok(hash.to_string())
}
...
mod tests {
    ...
    async fn test_register() {
        use crate::db::repository::MockCddRepository; use crate::db::models::User; use actix_web::App; let mut mock_repo = MockCddRepository::new();
        mock_repo.expect_create_user().returning(|_, _, _, _| Ok(User {
            id: 1, github_id: None, username: "test".into(), email: "test@test.com".into(), password_hash: None
        }));
        ...
        let req = actix_web::test::TestRequest::post()
            .uri("/auth/register")
            .set_json(RegisterPayload { username: "test".into(), email: "test@test.com".into(), password: Some("pwd".into()) })
            .to_request();
    ...
    async fn test_login_success() {
        ...
        mock_repo.expect_find_user_by_username().returning(move |_| Ok(Some(User {
            id: 1, github_id: None, username: "test".into(), email: "test@test.com".into(), password_hash: Some(hashed_pw.clone())
        })));
        ...
        let req = actix_web::test::TestRequest::post()
            .uri("/auth/login")
            .set_json(LoginPayload { username: "test".into(), password: Some("pwd".into()) })
            .to_request();
        let resp = actix_web::test::call_service(&app, req).await;
    ...
    async fn test_login_unauthorized() {
        use crate::db::repository::MockCddRepository;  use actix_web::App; let mut mock_repo = MockCddRepository::new();
        mock_repo.expect_find_user_by_username().returning(|_| Ok(None));
        ...
        let req = actix_web::test::TestRequest::post()
            .uri("/auth/login")
            .set_json(LoginPayload { username: "test".into(), password: Some("wrong".into()) })
            .to_request();
        let resp = actix_web::test::call_service(&app, req).await;
    ...
    async fn test_register_no_password() {
        ...
        let req = actix_web::test::TestRequest::post()
            .uri("/auth/register")
            .set_json(RegisterPayload { username: "test".into(), email: "test@test.com".into(), password: None })
            .to_request();
        let resp = actix_web::test::call_service(&app, req).await;
        assert_eq!(resp.status(), actix_web::http::StatusCode::CREATED);
    }
    ...
    async fn test_login_no_password() {
        ...
        let req = actix_web::test::TestRequest::post()
            .uri("/auth/login")
            .set_json(LoginPayload { username: "test".into(), password: None })
            .to_request();
        let resp = actix_web::test::call_service(&app, req).await;
        assert_eq!(resp.status(), actix_web::http::StatusCode::UNAUTHORIZED);
    }
